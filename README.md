...
- [ ] **模拟盘实跑验证** — 目标：跑赢沪深300至少3个月
- [ ] **观察 Qwen Skills 精排效果** — 确认不再 fallback，评估精排 vs 规则评分的差异
- [ ] **检查交易频率合理性** — 当前5分钟/轮，反转策略可能不需要这么频繁
- [ ] **验证两阶段执行(先卖后买)** — 确认卖出后资金释放正确
...
- [ ] **调仓频率自适应** — 震荡/熊市降低频率（日频），牛市可保持高频
- [ ] **止盈止损优化** — 当前止损-5%对反转策略可能太紧，反转需要更宽容的回撤忍度
- [ ] **基本面因子叠加** — PE/PB/ROE 作为第二层过滤，排除基本面恶化的反转陷阱
- [ ] **北向资金因子** — A 股特有的增量信息，可作为 regime 的第6个维度

### P2 — 系统完善

- [ ] **GGUF 导出** — 供 ollama 本地推理（非训练用）
- [ ] **DPO 偏好优化** — 交易日志积累3个月后，用实际收益做偏好对
- [ ] **日报生成** — 每日交易摘要 + 持仓变动 + 收益归因
- [ ] **监控告警** — 守护进程异常退出、连续失败、异常亏损时通知
